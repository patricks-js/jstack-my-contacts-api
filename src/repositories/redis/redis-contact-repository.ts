...
export class RedisContactRepository implements CacheRepository<Contact> {
  ...
  async set(keySuffix: string, data: Contact): Promise<void> {
  ...
  async setById(id: string, data: Contact): Promise<void> {
  ...
  async get(keySuffix: string): Promise<Contact | null | undefined> {
    const key = this.composeKey(keySuffix);
  ...
  async getById(id: string): Promise<Contact | null | undefined> {
    const key = this.composeKey(id);
    const data = await redisClient.get(key);
