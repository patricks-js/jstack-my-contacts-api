...
export class RedisCategoryRepository implements CacheRepository<Category> {
  ...
  async set(keySuffix: string, data: Category): Promise<void> {
  ...
  async setById(id: string, data: Category): Promise<void> {
  ...
  async get(keySuffix: string): Promise<Category | null | undefined> {
    const key = this.composeKey(keySuffix);
